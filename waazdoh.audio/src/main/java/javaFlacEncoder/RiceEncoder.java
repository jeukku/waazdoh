...
public class RiceEncoder {
	 ...
	 *            Set TRUE if using a five-bit parameter size, FALSE for a
	 *            four-bit parameter
	 * @param order
	 *            Specify order of partitions to be used(actual number of
	...
	public static int encodeRicePartitionEscaped(int[] values, int inputOffset,
		 ...
		 * too much to it. We *can* add another element to the given one if
		 * need.
		 */
		...
		System.err.println("LAST IN RICE : "
				+ Integer.toHexString(values[(inputCount - 1) * inputStep
						+ inputOffset]));
		for (int i = 0; i < inputCount; i++) {
			...
			EncodedElement.addInt(values[i * inputStep + inputOffset], 16,
					destOffset, data);
			destOffset += 16;
			bitsWritten += 16;
		}
	 ...
	 * This encodes a single rice-partition. In general, beginResidual(...)
	 * should be used before this method.
	 * 
	 * @param values
	 ...
	 * @param inputStep
	 *            number of values to skip between target values(for interleaved
	 *            data.
	 * @param inputCount
	 *            number of total values to encode
	 * @param bitParam
	 *            rice-parameter to use. This value should be based upon the
	 *            distribution of the input data(roughly speeking, each value
	 ...
	 * @param fiveBitParam
	 *            Set true if this header should use a five-bit rice-parameter,
	 *            false for a four bit parameter.
	...
	public int encodeRicePartition(int[] values, int inputOffset,
		...
		for (int i = 0; i < inputCount; i++) {
			...
			if (dataToEncodeSpaceNeeded + nextToEncode >= maxToEncode) {
				...
				destEle.packIntByBits(dataToEncode, bitsToEncode, 0,
						nextToEncode);
				nextToEncode = 0;
			}
			...
			int count = 0;
			while (upperBits > 0) {
				int tempVal = (upperBits > 32) ? 32 : upperBits;// can only
																// write 32 bits
																// at a time.
	 ...
	 * int inputStep, int inputCount, EncodedElement destEle, int bitParam,
	 * boolean fiveBitParam) {
	 * 
	 * //return RiceEncoder.encodeRicePartitionOld(values, inputOffset,
	 * inputStep, inputCount, destEle, bitParam, fiveBitParam); //return
	 * RiceEncoder.encodeRicePartitionPackInt(values, inputOffset, inputStep,
	 * inputCount, destEle, bitParam, fiveBitParam); //return
	 * this.encodeRicePartitionObjectDest(values, inputOffset, inputStep,
	 * inputCount, destEle, bitParam, fiveBitParam); return 0; }
	 */
