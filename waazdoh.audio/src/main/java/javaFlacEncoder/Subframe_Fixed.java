...
public class Subframe_Fixed extends Subframe {
	...
	public int encodeSamples(int[] samples, int count, int start, int skip,
		...
		// create space for results: Need four sets for the 5 different
		// versions,
		...
		for (int i = 1; i < 5; i++) {
			error1[i] = samples[start + i * increment]
					- samples[start + (i - 1) * increment];
			tempI = error1[i];
		...
		for (int i = 5; i < count; i++) {
			// error1[i] =
			// samples[start+i*increment]-samples[start+(i-1)*increment];
		...
		for (int i = 0; i < 5; i++) {
			...
			lowOrderBits[i] = (int) (Math.ceil(Math.log(tempLowOrderBits)
					/ LOG_2));
			if (lowOrderBits[i] < 1)
				lowOrderBits[i] = 1;
			else if (lowOrderBits[i] > sampleSize)
				lowOrderBits[i] = sampleSize;
			// lowOrderBits[i]++;//DOUBLE CHECK VALIDITY OF THIS. Decreases the
			// bits needed, but "shouldn't"
			// bits[i] =
			// (int)(Math.log(sum[i])/Math.log(10))*(count-1)+sampleSize*i;
		...
		_totalBits = unencSampleSize
				* order
				+ 8
				+ RiceEncoder.calculateEncodeSize(errors, errorOffset,
						errorStep, errorCount, lowOrderBits[order]);
		return encodedSamples;
	}

	/**
	 * Return the estimated size of the previous encode attempt in bits. Since
	 * returning the data from an encode is costly(due to the rice encoding and
	 * FLAC compliant bit-packing), this allows us to estimate the size first,
	 * and therefore choose another subframe type if this is larger.
	 * 
	...
	public EncodedElement getData(EncodedElement dataEle) {
		 ...
		 * errors[errorOffset+i*errorStep]; if(error >= 32767 || error <=
		 * -32767) System.err.println("Error Bound issue?: " + error); }
		 */
	...
	public int encodeSamples(int[] samples, int count, int start, int skip,
		...
		encodedSamples = encodeSamples(samples, count, start, skip, offset,
				unencSampleSize);
	 ...
	 * int[(int)Math.pow(2,maxPartitions)][]; for(int i = 0; i < maxPartitions;
	 * i++) { sums[i] = new int[(int)Math.pow(2, i)]; } int[] counts = new
	 * int[maxPartitions]; int[] usedPartitions = new int[maxPartitions]; int[]
	 * lastPartitionCount = new int[maxPartitions]; for(int i = 0; i <
	 * maxPartitions; i++) { counts[i] = errorCount/sums[i].length;
	 * if(errorCount % sums[i].length != 0) counts[i]++; usedPartitions[i] =
	 * errorCount/counts[i]; if(errorCount %counts[i] != 0) usedPartitions[i]++;
	 * } for(int i = 0; i < maxPartitions; i++) { int temp =
	 * errorCount+errorOffset/ } int[] sizes = new int[maxPartitions];
	 * 
	 * int temp = 0; double log2 = Math.log(2); for(int i = 0; i < errorCount;
	 * i++) { temp = errors[errorOffset+i*errorStep]; if(temp < 0) temp = -temp;
	 * for(int x = 0; x < maxPartitions; x++) { float destDiv = i/counts[x]; int
	 * destIndex = (int)(i/destDiv); sums[x][destIndex] += temp; }
	 * 
	 * } //sum up all bit sizes per partition, choose best size. for(int i = 0;
	 * i < maxPartitions; i++) { int tempTotal = 0; sizes[i] = 0;
	 * 
	 * for(int x = 0; x < usedPartitions[i]; x++) { tempTotal =
	 * (int)(Math.log(sums[i][x])/Math.log(2)); float destDiv =
	 * (float)errorCount/sums[x].length; int destCount =
	 * errorCount/sums[x].length; int destIndex = (int)(i/destDiv); sizes[i] +=
