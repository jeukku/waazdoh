...
public class EncodedElement_32 extends EncodedElement {
	...
	public static void addInt(int value, int count, int startPos, int[] dest) {
		 ...
		 * two cases to handle: 1) All bits fit in one index, appropriately
		 * shifted up first. Mask upper bits we'll merge with(Ones in upper).
		 * Mask lower bits we'll merge with(Ones in lower). destMask = OR Upper
		 * and lower masks together. currentIndex = currentIndex & destMask
		 * destMask = ~destMask; upshift by 32-count-currentOffset; inputValue =
		 * inputValue & destMask; currentIndex = currentIndex | inputValue 2)
		 * Some bits merge in top of one index, remaining bits merge in bottom
		 * of second index A) CurrentIndex Create inputValueHigh, with high
		 * order bits which will enter currentIndex. Handle this as we handled
		 * case one(pretend it's a new value) B) CurrentIndex++ We fill from
		 * upper edge. No upper mask needed. upshift value by 32-count; Mask
		 * lower bits we'll merge with(Ones in lower) currentIndex =
		 * currentIndex & destMask currentIndex = currentIndex | destMask
		 */
	...
	public static void packIntByBits(int[] inputA, int[] inputBits,
			int inputOffset, int countA, int startPosIn, int[] dest) {
		if (DEBUG_LEV > 30)
	...
	public EncodedElement convertToEncodedElement() {
		 ...
		 * 
		 * byteBase += 7; result[byteBase--] = (byte)(value0); value0 = value0
		 * >> 8; result[byteBase--] = (byte)(value0); value0 = value0 >> 8;
		 * result[byteBase--] = (byte)(value0); value0 = value0 >> 8;
		 * result[byteBase] = (byte)(value0);
		 * 
		 * byteBase += 7; result[byteBase--] = (byte)(value1); value1 = value1
		 * >> 8; result[byteBase--] = (byte)(value1); value1 = value1 >> 8;
		 * result[byteBase--] = (byte)(value1); value1 = value1 >> 8;
		 * result[byteBase] = (byte)(value1);
		 * 
		 * byteBase += 7; result[byteBase--] = (byte)(value2); value2 = value2
		 * >> 8; result[byteBase--] = (byte)(value2); value2 = value2 >> 8;
		 * result[byteBase--] = (byte)(value2); value2 = value2 >> 8;
		 * result[byteBase] = (byte)(value2);
		 * 
		 * byteBase += 7; result[byteBase--] = (byte)(value3); value3 = value3
		 * >> 8; result[byteBase--] = (byte)(value3); value3 = value3 >> 8;
		 * result[byteBase--] = (byte)(value3); value3 = value3 >> 8;
		...
		for (int i = 3; i < data_32.length * 4; i += 7) {
			...
			/*
			 * result[byteBase++] = (byte)(value >> 24); result[byteBase++] =
			 * (byte)(value >> 16); result[byteBase++] = (byte)(value >> 8);
