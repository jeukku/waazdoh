...
public class Subframe_LPC extends Subframe {
	...
	public int encodeSamples(int[] samples, int count, int start, int skip,
		...
		for (int i = maxOrder; i >= minOrder; i--) {
			...
			int tempTotalBits = partialEncodeLPC(samples, count, start,
					increment, lpcs[i], this, frameSampleSize);
	...
	public EncodedElement getData() {
		...
		writeLPC(_samples, _lastCount, _start, _increment, result,
				_frameSampleSize, _lowOrderBits, _precision, _shift,
	 ...
	 * @param order
	 *            number of values to quantize. First value skipped,
	 *            coefficients array must be at least order+1 in length.
	 * @param precision
	 *            number of signed bits to use for coefficients(must be in range
	 *            2-15, inclusive).
	 * @return
	 */
	private static int quantizeCoefficients(double[] coefficients, int[] dest,
		...
		int shiftApplied = 0;
		int maxValAllowed = (1 << (precision - 2)) - 1;// minus an extra bit for
														// sign.
	...
	private static void writeLPC(int[] samples, int count, int start,
			int increment, EncodedElement ele, int frameSampleSize,
			int riceParam, int precision, int shift, int[] coeffs,
			int[] errors, int order, RiceEncoder rice) {
		writeHeadersAndData(ele, order, coeffs, precision, shift, samples,
				frameSampleSize, start, increment - 1);
	...
	private static int partialEncodeLPC(int[] samples, int count, int start,
		...
		// and resulting in smaller files. win-win. On second thought, that
		// can't
		// be why it's quicker. The profile is showing *more* invocatiosn of
		// this
		 ...
		 * if(Double.isNaN(deviation)) System.err.println("deviation is NaN");
		 * System.err.println("Error: "+(int)error/count); System.exit(0); }
		 * if(tempLowOrderBits < 1) tempLowOrderBits = 1; else if
		 * (tempLowOrderBits > frameSampleSize) { tempLowOrderBits =
		 * frameSampleSize; }
		 */
		int precision = 15;
		...
		int shift = quantizeCoefficients(lpc.rawCoefficients, coeffs, order,
				precision);
		...
		for (int i = order; i < count; i++) {
			...
			temp = temp >> shift;// with precision fixed at 15, this should
									// always
									// shift back down to int territory for
									// bitsize <=24
			lpcSubframe._errors[i] = samples[targetSampleBase] - (int) temp;
		}
		tempLowOrderBits = getParam(lpcSubframe._errors, count, order,
				frameSampleSize);
