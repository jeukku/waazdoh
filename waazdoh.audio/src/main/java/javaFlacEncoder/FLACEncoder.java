...
public class FLACEncoder {
	 ...
	 */
	private int MAX_THREADED_FRAMES = Runtime.getRuntime()
			.availableProcessors();
	 ...
	 * officially closed. Must use a streamLock to set and get this. Some
	 * actions must not be taken while a stream is opened
	 */
	...
	/*
	 * Stores unfilled BlockEncodeRequest(not ready for queue, unless ending
	 * stream
	 */
	...
	/*
	 * threagedFrames keeps track of frames given to threadManager. We must
	 * still update the configurations of them as needed. If we ever create new
	 ...
	 * encoding thread. IOException temporarily stored by childException when
	 * this is true.
	 */
	boolean error = false;
	/*
	...
	public FLACEncoder() {
		...
		try {
			...
			throw new IllegalStateException(
					"Error! FLACEncoder cannot function"
	 ...
	 * configuration is not stored by this object, but instead copied. This is
	 * to prevent the alteration of the config during an encode process. This
	 ...
	 *         cannot be altered(such as if another thread is currently
	 *         encoding).
	 */
	 ...
	 * configuration is not stored by this object, but instead copied. This is
	 * to prevent the alteration of the config during an encode process. This
	 * method must not be called in the middle of a stream, stream contents may
	 * become invalid. Calling this method clears any data stored by this
	...
	public boolean setStreamConfiguration(StreamConfiguration sc) {
		...
		if (sc != null) {
			if (flacStreamIsOpen || isEncoding)
				...
				streamLock.lock();
				try {
					if (flacStreamIsOpen || isEncoding) {// can't change
															// streamconfig on
															// open stream.
						changed = false;
					} else {
	 ...
	 * stream. Does *not* clear any stored, unwritten data. To flush stored
	 * samples, call clear().
	 */
	private void reset() {
		md5.getMD().reset();
		if (flacWriter != null)
			flacWriter = new FLACStreamController(
					flacWriter.getFLACOutputStream(), streamConfig);
	}

	/**
	 * Clear all samples stored by this object, but not yet encoded. Should be
	 * called between encoding differrent streams(before more samples are
	 * added), unless you desire to keep unencoded samples. This does NOT reset
	...
	private void closeFLACStream() throws IOException {
		...
		try {
			if (!flacStreamIsOpen)
				throw new IllegalStateException(
						"Cannot close a non-opened stream");
	...
	/**
	 * Begin a new FLAC stream. Prior to calling this, you must have already set
	 * the StreamConfiguration and the output stream, both of which must not
	 ...
	 *         result if "count" is set to a size that is too large to be valid
	 *         with the given array and current configuration.
	 */
	public void addSamples(int[] samples, int count) {
		...
		try {
			...
			int offset = 0;
			while (remaining > 0) {
				int newRemaining = unfilledRequest.addInterleavedSamples(
						samples, offset, remaining, maxBlock);
	...
	private void writeFinishedBlock(BlockEncodeRequest ber) throws IOException {
		...
		if (threadManager.getTotalManagedCount() == 1) {// this is the final
														// block
			streamLock.lock();
			try {
	 ...
	 * back in the same order the encoder sent them out. This is intended to be
	 * used in threading mode only at the moment(sending them to a
	...
	/**
	 * Attempt to Encode a certain number of samples(threaded version). Encodes
	 * as close to count as possible. Uses multiple threads to speed up
	 * encoding. If getThreadCount() <= 0, simply calls the non-threaded
	 * version, encodeSamples(...), and blocks until it returns.
	 * 
	 * @param count
	 *            number of samples to attempt to encode. Actual number encoded
	 *            may be greater or less if count does not end on a block
	 *            boundary. If "end" is false, we may set this value to
	 *            something absurdly high, such as Integer.MAX_VALUE to ensure
	 *            all available, full blocks are encoded.
	 * 
	 * @param end
	 *            true to finalize stream after encode, false otherwise. If set
	 *            to true, and return value is greater than or equal to given
	 *            count, no more encoding must be attempted until a new stream
	 *            is began.
	 * 
	 * @param blockingCount
	 *            value is used for flow-control into this encoder. This method
	 *            will block until fewer than the given number of blocks remain
	 *            queued for encoding.
	 * 
	 * @return number of samples encoded. This may be greater or less than
	 *         requested count if count does not end on a block boundary. This
	 *         is NOT an error condition. If end was set "true", and returned
	 *         count is less than requested count, then end was NOT done, if you
	 *         still wish to end stream, call this again with end true and a
	 *         count of of <= samplesAvailableToEncode()
	 * 
	 * @throws IOException
	 *             if there was an error writing the results to output stream.
	 */
	public int t_encodeSamples(final int inCount, final boolean end,
			int blockingCount) throws IOException {
		int count = inCount;
		if (MAX_THREADED_FRAMES <= 0)
			return encodeSamples(count, end);
		int encodedCount = 0;
		if (end)
			sampleLock.lock();// lock to avoid race condition in unfinishedBlock
								// section.
		try {
			checkForThreadErrors();
			streamLock.lock();
			...
			if (end) {
				streamLock.lock();
				try {
					if (count > 0 && unfilledRequest != null
							&& unfilledRequest.count >= count) {
			...
			// handle "end" setting
			if (end && encodedCount >= inCount) {// close if all requests were
													// written.
				closeFLACStream();
			}
		} finally {
		...
		try {
			...
			while (count > 0 && preparedRequests.size() > 0 && !encodeError) {
				...
				if (encodedSamples < 0) {
					...
					System.err
							.println("FLACEncoder::encodeSamples : Error in encoding");
					encodeError = true;
					break;
				}
			...
			if (end) {
				if (threadManager != null)
					threadManager.stop();
				// if(end && !encodeError && this.samplesAvailableToEncode() >=
				// count) {
				if (count > 0 && unfilledRequest != null
						&& unfilledRequest.count >= count) {
					...
					int encodedSamples = encodeRequest(ber, channels);
					if (encodedSamples < 0) {
						// ERROR! Return immediately. Do not add results to
						// output.
						System.err
								.println("FLACEncoder::encodeSamples : (end)Error in encoding");
						count = -1;
					} else {
						count -= encodedSamples;
						encodedCount += encodedSamples;
				...
				if (DEBUG_LEV > 30)
					System.err
							.println("End set but not done. Error possible. "
	...
	private int encodeRequest(BlockEncodeRequest ber, int channels)
			throws IOException {
		ber.frameNumber = flacWriter.incrementFrameNumber();
		...
		int encoded = frame.encodeSamples(block, encodedSamples, 0,
				channels - 1, result, ber.frameNumber);
