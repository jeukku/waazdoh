...
public class FLAC_FileEncoder {
	...
	public enum Status {
		...
		/**
		 * internal error is something that went haywire that was discovered due
		 * to internal sanity checks. A problem in API.
		 */
		INTERNAL_ERROR,
	 ...
	 * @param outputFile
	 *            Output file to write FLAC stream to. If file exists, it will
	...
	public Status encode(File inputFile, File outputFile) {
		...
		try {
			...
			// open stream
			openStream();
			AudioStreamEncoder.encodeAudioInputStream(sin, MAX_READ, flac,
					useThreads);
		} catch (IOException e) {
			status = Status.FILE_IO_ERROR;
