...
public class AudioStreamEncoder {
	 ...
	 * FLACEncoder must be in a state to accept samples and
	 * encode(FLACOutputStream, EncodingConfiguration, and StreamConfiguration
	 * have been set, and FLAC stream has been opened).
	 * 
	 * @param sin
	 * @return
	 * @throws IOException
	 * @throws IllegalArgumentException
	 *             thrown if input sample size is not supported
	 */
	public static int encodeAudioInputStream(AudioInputStream sin, int maxRead,
			FLACEncoder flac, boolean useThreads) throws IOException,
		...
		if (sampleSize % 8 != 0) {
			...
			throw new IllegalArgumentException(
					"Unsupported Sample Size: size = " + sampleSize);
		}
			...
			if (bigEndian) {
				...
				for (int i = 0; i < framesRead * channels; i++) {
					...
					for (int x = 0; x < bytesPerSample; x++) {
						int upShift = 8 * x;
						if (x == bytesPerSample - 1 && isSigned)// don't
																// mask...we
																// want sign
							temp = ((samplesIn[bytesPerSample * i + x]) << upShift);
						else
	 ...
	 * @return Bit positions set according to issue: Bit Position : Problem-area
	 *         0 : Channel count unsupported 1 : Sample size unsupported 2 :
	 *         Sample Rate unsupported 3 : Encoding Type Unsupported 4-7:
	 *         unused, always zero. return value of 0 means supported
	 */
	public static int getDataFormatSupport(AudioFormat format) {
		...
		if (!(AudioFormat.Encoding.ALAW.equals(encoding)
				...
				|| AudioFormat.Encoding.PCM_SIGNED.equals(encoding) || AudioFormat.Encoding.PCM_UNSIGNED
					.equals(encoding)))
