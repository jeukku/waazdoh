...
public abstract class Subframe {
	...
	/**
	 * StreamConfiguration used for encoding. The same StreamConfiguration MUST
	 * be used throughout an entire FLAC stream
	 */
	 ...
	 * may include both left and right samples of a stereo stream, while this
	 * SubFrame is only encoding the 'right' channel(channel 2). Therefore,
	 * â€œskipâ€� would equal 2, resulting in the valid indices being only those
	 * where â€œindex mod 2 equals 1â€�.
	 * 
	 * @param samples
	 ...
	 *            EncodedElement given is not altered. New data is attached
	 *            starting with â€œdata.getNext()â€�. If â€œdataâ€� already has
	 *            a â€œnextâ€� set, it will be lost!
	 * @param offset
	 * @param bitsPerSample
	 *            Number of bits per single-channel sample. This may differ from
	 *            the StreamConfiguration's sample size, depending on the
	 *            subframe used(i.e, the "side-channel" of a FLAC stream uses
	 *            one extra bit compared to the input channels).
	 * 
	 * @return number of encoded samples, or negative value indicating an error
	 *         has occurred.
	 * 
	 */
	public abstract int encodeSamples(int[] samples, int count, int start,
			int skip, EncodedElement data, int offset, int bitsPerSample);

	/*
	 * public abstract int encodeSamples(int[] samples, int count, int start,
	 * int skip, int offset, int bitsPerSample);
	 * 
