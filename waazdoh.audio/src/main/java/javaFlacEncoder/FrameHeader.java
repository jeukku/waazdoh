...
public class FrameHeader {
	...
	private static final int definedBlockSizes[] = { -1, 192, 576, 1152, 2304,
			4608, -1, -1, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768 };

	private static final int definedSampleRates[] = { 0, 88200, 176400, 192000,
			8000, 16000, 22050, 24000, 32000, 44100, 48000, 96000, -1, -1, -1,
			-1 };
	 ...
	 * headers. We can't use static functions to do this, since the process uses
	 * a CRC8 object which must be instantiated.
	 * 
	 */
	public FrameHeader() {
	 ...
	 * 
	 * @param fixBlock
	 *            True to use a fixed block size, false to use variable. At this
	 *            time, this *must* be set to True, as variable block size is
	 *            not yet implemented.
	 * @param blockSize
	 ...
	 * @param channelAssign
	 *            Channel assignment used in this frame's encoding. See
	 *            EncodingConfiguration class documentation for more
	 *            information.
	 * @param sampleSize
	 *            Bits per sample.
	 * @param frameNumber
	 *            For fixed block-size encodings, this is the frame-number
	 *            starting at zero and incrementing by one. For variable block
	 *            encodings, this is the sample number of the first sample in
	 *            the frame.
	 * @param channelCount
	...
	public EncodedElement createHeader(boolean fixBlock, int blockSize,
		...
		byte[] encodedFrameNumber = UTF8Modified
				.convertToExtendedUTF8(frameNumber);
		// set block size bits
		...
		crcCalculator.reset();
		crcCalculator
				.updateCRC8(result.getData(), 0, result.getTotalBits() / 8);
