...
public class EncodedElement {
	 ...
	 * zero of first byte. offset must always be less than or equal to
	 * usableBits.
	 */
	 ...
	 * 
	 *            Pre-condition: None Post-condition: 'data' is now stored by
	...
	/**
	 * Add a number of bits from a long to the end of this list's data. Will add
	 * a new element if necessary. The bits stored are taken from the lower-
	 * order of input.
	 * 
	 * @param input
	...
	/**
	 * Add a number of bits from an int to the end of this list's data. Will add
	 * a new element if necessary. The bits stored are taken from the lower-
	 * order of input.
	 * 
	 * @param input
	...
	public EncodedElement addInt(int input, int bitCount) {
		if (next != null) {
			...
			// At this point, we have the space, and we are the end of the
			// chain.
			int startPos = this.usableBits;
			byte[] dest = this.data;
			 ...
			 * bitCount, startPos, dest); else EncodedElement.addInt_new(input,
			 * bitCount, startPos, dest);
			 */
			usableBits += bitCount;
			return this;
		}
	 ...
	 * @param skip
	 *            number of indices to skip between values(in case input data is
	 *            interleaved with non-desirable data).
	 * @param countA
	 *            Number of total indices to store from.
	 * @return EncodedElement containing end of packed data. Data may flow
	 *         between multiple EncodedElement's, if an existing element was not
	 *         large enough for all values.
	 */
	public EncodedElement packInt(int[] inputArray, int bitSize, int start,
			int skip, int countA) {
		...
		EncodedElement.packInt(inputArray, bitSize, usableBits, start, skip,
				countA, data);
		...
		if (countA > 0) {
			...
			next = new EncodedElement(size, tOff);
			// add int to child
			return next.packInt(inputArray, bitSize, start + writeCount
					* (skip + 1), skip, countA);
		} else {
	 ...
	 *         large enough for all values.
	 */
	public EncodedElement packIntByBits(int[] inputA, int[] inputBits,
			int inputOffset, int countA) {
		// go to end if we're not there.
		if (next != null) {
			EncodedElement end = EncodedElement.getEnd_S(next);
	...
	/**
	 * public static void addIntOld(int input, int count, int startPos, byte[]
	 * dest) { if(DEBUG_LEV > 30)
	 * System.err.println("EncodedElement::addInt : Begin"); int currentByte =
	 * startPos/8; int currentOffset = startPos%8; int bitRoom;//how many bits
	 * can be placed in current byte int upMask;//to clear upper bits(lower bits
	 * auto-cleared by L-shift int downShift;//bits to shift down, isolating top
	 * bits of input int upShift;//bits to shift up, packing byte from top.
	 * while(count > 0) { //find how many bits can be placed in current byte
	 * bitRoom = 8-currentOffset; //get those bits //i.e, take upper
	 * 'bitsNeeded' of input, put to lower part of byte. downShift =
	 * count-bitRoom; upMask = 255 >>> currentOffset; upShift = 0; if(downShift
	 * < 0) { //upMask = 255 >>> bitRoom-count; upShift = bitRoom - count;
	 * upMask = 255 >>> (currentOffset+upShift); downShift = 0; } if(DEBUG_LEV >
	 * 30) { System.err.println("count:offset:bitRoom:downShift:upShift:" +
	 * count+":"+currentOffset+":"+bitRoom+":"+downShift+":"+upShift); } int
	 * currentBits = (input >>> downShift) & (upMask); //shift bits back up to
	 * match offset currentBits = currentBits << upShift; upMask = (byte)upMask
	 * << upShift;
	 * 
	...
	/**
	 * public static void packIntOLD_WORKING(int[] inputArray, int startBitSize,
	 * int startPos, int start, int skip, int count, byte[] dest) { if(DEBUG_LEV
	 * > 0) System.err.println("EncodedElement::packInt : Begin"); if(DEBUG_LEV
	 * > 10) System.err.println("start:skip:count : "
	 * +start+":"+skip+":"+count); for(int i = 0; i < count; i++) {
	 ...
	 *            Number of total indices to store from.
	 * @param startPosIn
	 *            First usable index in destination array(byte index =
