...
public class BlockThreadManager implements Runnable {
	...
	public BlockThreadManager(FLACEncoder encoder) {
		...
		frameThreadMap = Collections
				.synchronizedMap(new HashMap<FrameThread, Thread>());
	 ...
	 * returning the FrameThread object to the manager for use in future
	 * Threads.
	 * 
	 * @param ft
	...
	public BlockEncodeRequest getWaitingRequest() {
		...
		synchronized (getLock) {
			...
			try {
				while (loop) {
					result = unassignedEncodeRequests.poll(500,
	...
	public void run() {
		...
		try {
			while (loop) {
				try {
					if (nextTarget == null)
						nextTarget = orderedEncodeRequests.poll(500,
								TimeUnit.MILLISECONDS);
					if (nextTarget == null) {
						loop = false;
						...
						BlockEncodeRequest ber = finishedEncodeRequests.poll(
								500, TimeUnit.MILLISECONDS);
						if (ber == null) {// nothing to process yet, let this
											// thread end.
							loop = false;
	 ...
	 * assign a request to an encoding thread(which may not occur if no threads
	 * are currently available). Requests are passed back to the currently set
	...
	synchronized public boolean addRequest(BlockEncodeRequest ber) {
		// add request to the manager(requests are automatically removed when
		// complete)
		process = true;
		boolean r = true;
		try {
