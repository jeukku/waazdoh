...
public class EncodingConfiguration implements Cloneable {
	 ...
	 * channels refers to stereo audio as expected. INDEPENDENT channels refer
	 * to each channel encoded separately. SIDE channel is the difference of the
	 * LEFT and RIGHT channels(LEFT-RIGHT). MID channel is the integer average
	 * of LEFT and RIGHT channels( (LEFT+RIGHT)/2 ). Options using MID and/or
	 * SIDE channels may benefit encoding by taking advantage of similarities
	 * between the channels, and are available in FLAC for STEREO streams only.
	 * In general, ENCODER_CHOICE should be chosen.
	 */
	...
	/**
	 * Set the subframe type to use. This refers to the way each
	 * subframe(channel) is compressed. See documentation for
	 * {@link SubframeType SubframeType} for more info on choices.
	 * 
	 * @param type
	 */
	...
	public void setMinLPCOrder(int order) {
		...
		minimumLPCOrder = (minimumLPCOrder > MAX_LPC_ORDER) ? MAX_LPC_ORDER
				: minimumLPCOrder;
	}
	...
	public void setMaxLPCOrder(int order) {
		...
		maximumLPCOrder = (maximumLPCOrder > MAX_LPC_ORDER) ? MAX_LPC_ORDER
				: maximumLPCOrder;
	}
