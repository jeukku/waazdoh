...
public class FLAC_ConsoleFileEncoder {
	...
	/**
	 * flag to specify subframe type to use. Must be followed by a valid type as
	 * given by the fields TYPE_*.
	 */
	...
	public static final String MIN_BLOCK_ERROR = "Error with minimum block "
			+ "size: integer value between "
			+ StreamConfiguration.MIN_BLOCK_SIZE + " & "
			+ StreamConfiguration.MAX_BLOCK_SIZE + " must follow \""
			+ MIN_BLOCK + "\"";
	...
	public FLAC_ConsoleFileEncoder(String[] args) {
		...
		if (inputFile != null && outputFile != null && attemptEncode)
			...
			System.err
					.println("error: inputFile not given or an error occured");
			attemptEncode = false;
		} else if (attemptEncode) {
			System.err.println(inputFile.getName());
			System.err
					.println("error: outputFile not given or an error occured");
			attemptEncode = false;
		}
		if (attemptEncode)
	...
	private boolean getOutputFile(String filename) {
		...
		/*
		 * if(file.exists() ) {
		 * System.err.println("Output file exists, exiting"); result = false; }
		 */
		{
			try {
				if (!file.exists())
				...
				if ((result &= file.canWrite()) == false) {
					System.err.println("Error, can't write to output file: "
							+ filename);
				} else {
					outputFile = file;
				}
