...
public class FLAC_MD5 {
	...
	/**
	 * Add samples to the MD5 hash. CURRENTLY ONLY MAY WORK FOR: sample sizes
	 * which are divisible by 8. Need to create some audio to test with.
	 * 
	 * @param samples
	 * @param count
	 * @param channels
	...
	public void addSamplesToMD5(int[] samples, int count, int channels,
		...
		if (sampleSize % 8 != 0)
			bytesPerSample++;
		if (_dataMD5 == null
				|| _dataMD5.length < count * bytesPerSample * channels) {
	 ...
	 * with.
	 */
	private static final void splitSamplesToBytes(int[] samples,
			int totalSamples, int bytesPerSample, byte[] dataMD5) {
		int destIndexBase = 0;
		int i = 0;
