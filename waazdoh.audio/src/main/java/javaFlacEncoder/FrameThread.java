...
public class FrameThread implements Runnable {
	 ...
	 * @param manager
	 *            BlockThreadManager to use as the BlockEncodeRequest source and
	 *            destination.
	 */
		...
		synchronized (this) {
			...
			while (ber != null && process) {
				if (ber.frameNumber < 0) {
					...
					ber.encodedSamples = frame.encodeSamples(ber.samples,
							ber.count, ber.start, ber.skip, ber.result,
							ber.frameNumber);
					ber.valid = true;
