...
public class UTF8Modified {
	...
	static long[] limits = { oneByteLimit, twoByteLimit, threeByteLimit,
			fourByteLimit, fiveByteLimit, sixByteLimit, sevenByteLimit };
	 ...
	 * 
	 * @param value
	 *            value to convert to extended UTF8(value must be positive and
	 *            36 bits or less in size)
	...
	public static byte[] convertToExtendedUTF8(long value) {
		...
		while (bytesLeft > 1) {
			...
			int val = ((int) (value >>> inputIndex) & midByteMask)
					| midByteMarker;
		...
		if (DEBUG_LEV > 10) {
			System.err.print("input:result_length:result :: " + value + ":"
					+ finalResult.length + "::");
