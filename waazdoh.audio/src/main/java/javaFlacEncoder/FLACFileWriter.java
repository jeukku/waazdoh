...
public class FLACFileWriter extends AudioFileWriter {
	 ...
	 * 
	 * @see
	 * javax.sound.sampled.spi.AudioFileWriter#getAudioFileTypes(javax.sound
	...
	public int write(AudioInputStream stream, Type fileType, File out)
		...
		if (!flacTypes[0].equals(fileType)) {
			throw new IllegalArgumentException("File type " + fileType
					+ " not supported.");
		...
		try {
			...
			AudioStreamEncoder.encodeAudioInputStream(stream, MAX_READ, flac,
					false);

		} finally {
			if (flacFileOutputStream != null) {
				bytesWritten = Long.valueOf(flacFileOutputStream.size())
						.intValue();
	...
	public int write(AudioInputStream stream, Type fileType, OutputStream out)
		...
		try {
			...
			if (flacStreamOutputStream != null) {
				bytesWritten = Long.valueOf(flacStreamOutputStream.size())
						.intValue();
