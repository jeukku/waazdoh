 ...
 * calculations needed for preparing the â€œchannel configurationâ€� used in
...
public class Frame {
	 ...
	 * in the given â€œdataâ€� EncodedElement list. Encodes 'count' samples,
	 * from index 'start', to index 'start' times 'skip', where â€œskipâ€� is
	 * the format that samples may be packed in an array. For example, 'samples'
	 * may include both left and right samples of a stereo stream. Therefore,
	 * â€œskipâ€� would equal 2, resulting in the valid indices for the first
	 * channel being even, and second being odd.
	 * 
	 * @param samples
	 ...
	 *            with â€œdata.getNext()â€�. If â€œdataâ€� already has a
	 *            â€œnextâ€� set, it will be lost!
	 ...
	 *         samples in this block, but the return value is â€œ4000â€�. There
	 *         is always an equal number of samples encoded from each channel.
	 *         This exists primarily to support dynamic block sizes in the
	 *         future; Pre-condition: none Post-condition: Argument 'data' is
	 *         the head of a list containing the resulting, encoded data stream.
	 */
	public int encodeSamples_OLD(int[] samples, int count, int start, int skip,
		...
		if (DEBUG_LEV > 0) {
			...
			if (DEBUG_LEV > 10) {
				System.err.println("\tsamples.length:" + samples.length
						+ ":count:" + count + ":start:" + start + ":skip:"
						+ skip + ":frameNumber:" + frameNumber);
			}
		}
		...
		if (chConf == EncodingConfiguration.ChannelConfig.INDEPENDENT) {
			...
			data = new EncodedElement(100, 0);
			int size = Frame.encodeMidSide(samples, count, start, skip, data,
					0, this);
			...
			data = new EncodedElement(100, 0);
			int size = allChannelDecorrelation(samples, count, start, skip,
					data, 0, this);
		...
		EncodedElement header = frameHeader.createHeader(true, count,
				sc.getSampleRate(), chConf, sc.getBitsPerSample(), frameNumber,
				channels, new EncodedElement(128, 0));
	...
	private EncodingConfiguration.ChannelConfig determineConfigUsed(
	...
	public int encodeSamples(int[] samples, int count, int start, int skip,
		...
		testConstant = true;
		EncodedElement data = null;
		ChannelData[][] chanConfigData = this.getChannelsToEncode(samples,
				count, sc.getChannelCount(), sc.getBitsPerSample());
		...
		// create header element; attach to result
		EncodedElement header = new EncodedElement(FrameHeader.MAX_HEADER_SIZE,
				0);
	...
	int encodeChannels(ChannelData[] channels, EncodedElement result) {
		...
		for (int i = 0; i < channels.length; i++) {
			EncodedElement temp = new EncodedElement();
			channelLength = encodeChannel(channels[i].getSamples(), count, 0,
					0, offset, temp, channels[i].getSampleSize());
	...
	int encodeIndependent(int[] samples, int count, int start, int skip,
			EncodedElement result, int offset) {
		if (DEBUG_LEV > 0) {
			System.err.println("Frame::encodeIndependent : Begin");
			System.err.println("start:skip:offset::" + start + ":" + skip + ":"
					+ offset);
		}
		...
		for (int i = 0; i < channelCount; i++) {
			...
			if (i == 1
				...
				channelBitsPerSample++;
					...
					&& chConf == EncodingConfiguration.ChannelConfig.RIGHT_SIDE)
				channelBitsPerSample++;
	...
	int encodeChannel(int[] samples, int count, int start, int skip,
			int offset, EncodedElement data, int channelBitsPerSample) {
		if (DEBUG_LEV > 0)
		...
		if (subframeType == EncodingConfiguration.SubframeType.VERBATIM) {
			...
			if (testConstant) {
				...
				conCount = constantSubframe.encodeSamples(samples, count,
						start, skip, constantEle, offset, channelBitsPerSample);
			}
			...
			if (conCount == count) {
				...
				int verbatimSize = verbatimSubframe.estimateSize(count,
						channelBitsPerSample);
				fixedSubframe.encodeSamples(samples, count, start, skip,
						offset, channelBitsPerSample);
				int fixedSize = fixedSubframe.estimatedSize();
				// int fixedSize = fixedSubframe.getEncodedSize();
				lpcSubframe.encodeSamples(samples, count, start, skip, offset,
						channelBitsPerSample);
...
//					System.err.println("Running verbatim");
					...
					size = lpcSubframe.getEncodedSize();
					if (size > lpcSize)
						System.err.println("Lpc size wrong: exp:real : "
								+ lpcSize + ":" + size);
				} else {// fixed
					smallest = fixedSubframe.getData();
					size = fixedSubframe.getEncodedSize();
					if (size > fixedSize)
						System.err.println("Fixed size wrong: exp:real : "
								+ fixedSize + ":" + size);
				}
			}
	...
	private static void getIndependentChannels(int[] samples, int count,
		...
		for (int i = 0; i < channelCount; i++) {
			...
			if (independentSamples[i] == null
					|| independentSamples[i].length < count)
			...
			if (temp != null)
				...
				channels[i] = new ChannelData(independentSamples[i], count,
						sampleSize, ChannelData.ChannelName.INDEPENDENT);
		}
	...
	private static void getMidSideChannels(int[] samples, int count,
		...
		if (midData != null)
			channels[0].setData(midSamples, count, sampleSize,
					ChannelData.ChannelName.MID);
		else
			channels[0] = new ChannelData(midSamples, count, sampleSize,
					ChannelData.ChannelName.MID);
		if (sideData != null)
			channels[1].setData(sideSamples, count, sampleSize + 1,
	...
	private ChannelData[][] getChannelsToEncode(int[] samples, int count,
	...
	private static int encodeMidSide(int[] samples, int count, int start,
			int skip, EncodedElement data, int offset, Frame f) {
	...
	private static double getVariance(double mean, int[] samples, int count,
	...
	private static int allChannelDecorrelation(int[] samples, int count,
			int start, int skip, EncodedElement data, int offset, Frame f) {
		if (DEBUG_LEV > 0) {
