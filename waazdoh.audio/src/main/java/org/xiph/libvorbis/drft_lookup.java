...
class drft_lookup {
				...
				for (k1 = 0; k1 < nfm1; k1++) {
					...
					for (j = 0; j < ipm; j++) {
						...
						for (ii = 2; ii < ido; ii += 2) {
							...
							arg = fi * argld;
							wa[_n + (i++)] = new Double(Math.cos(arg))
									.floatValue();
							wa[_n + (i++)] = new Double(Math.sin(arg))
									.floatValue();
						}
						is += ido;
					}
		...
		for (k1 = 0; k1 < nf; k1++) {
			...
			while (state != LDONE) {
				...
				case L101: {
					...
					if (na != 0)
						dradf4(ido, l1, ch, c, wa, waoff + iw - 1, waoff + ix2
								- 1, waoff + ix3 - 1);
					else
						dradf4(ido, l1, c, ch, wa, waoff + iw - 1, waoff + ix2
								- 1, waoff + ix3 - 1);
					state = L110;
					break;
				}
				...
				case L104: {
					...
					dradfg(ido, ip, l1, idl1, c, c, c, ch, ch, wa, waoff + iw
							- 1);
					na = 1;
					state = L110;
					break;
				...
				case L109: {
					dradfg(ido, ip, l1, idl1, ch, ch, ch, c, c, wa, waoff + iw
							- 1);
					na = 0;
					state = L110;
					break;
		...
		if (ido != 2) {
			...
			for (k = 0; k < l1; k++) {
				...
				for (i = 2; i < ido; i += 2) {
					...
					t5 += 2;
					t6 += 2;
					tr2 = wa1[waoff + i - 2] * cc[t3 - 1] + wa1[waoff + i - 1]
							* cc[t3];
					ti2 = wa1[waoff + i - 2] * cc[t3] - wa1[waoff + i - 1]
							* cc[t3 - 1];
					ch[t6] = cc[t5] + ti2;
					ch[t4] = ti2 - cc[t5];
					ch[t6 - 1] = cc[t5 - 1] + tr2;
	...
	static void dradf4(int ido, int l1, float[] cc, float[] ch, float[] wa,
		...
		if (ido != 2) {
			...
			for (k = 0; k < l1; k++) {
				...
				for (i = 2; i < ido; i += 2) {
					...
					t3 += t0;
					cr2 = wa[wa1 + i - 2] * cc[t3 - 1] + wa[wa1 + i - 1]
							* cc[t3];
					ci2 = wa[wa1 + i - 2] * cc[t3] - wa[wa1 + i - 1]
							* cc[t3 - 1];
					t3 += t0;
					cr3 = wa[wa2 + i - 2] * cc[t3 - 1] + wa[wa2 + i - 1]
							* cc[t3];
					ci3 = wa[wa2 + i - 2] * cc[t3] - wa[wa2 + i - 1]
							* cc[t3 - 1];
					t3 += t0;
					cr4 = wa[wa3 + i - 2] * cc[t3 - 1] + wa[wa3 + i - 1]
							* cc[t3];
					ci4 = wa[wa3 + i - 2] * cc[t3] - wa[wa3 + i - 1]
							* cc[t3 - 1];
				...
				for (j = 1; j < ip; j++) {
					...
					for (k = 0; k < l1; k++) {
						...
						for (i = 2; i < ido; i += 2) {
							idij += 2;
							t3 += 2;
							ch[t3 - 1] = wa[waoff + idij - 1] * c1[t3 - 1]
									+ wa[waoff + idij] * c1[t3];
							ch[t3] = wa[waoff + idij - 1] * c1[t3]
									- wa[waoff + idij] * c1[t3 - 1];
						}
					}
				}
			} else {
		...
		for (k1 = 0; k1 < nf; k1++) {
			...
			while (state != LDONE) {
				...
				case L101: {
					...
					if (na != 0)
						dradb4(ido, l1, ch, c, wa, waoff + iw - 1, waoff + ix2
								- 1, waoff + ix3 - 1);
					else
						dradb4(ido, l1, c, ch, wa, waoff + iw - 1, waoff + ix2
								- 1, waoff + ix3 - 1);
					na = 1 - na;
					state = L115;
					break;
				...
				case L106: {
					...
					ix2 = iw + ido;
					if (na != 0)
						dradb3(ido, l1, ch, c, wa, waoff + iw - 1, waoff + ix2
								- 1);
					else
						dradb3(ido, l1, c, ch, wa, waoff + iw - 1, waoff + ix2
								- 1);
					na = 1 - na;
					state = L115;
					break;
				...
				case L109: {
					...
					if (na != 0)
						dradbg(ido, ip, l1, idl1, ch, ch, ch, c, c, wa, waoff
								+ iw - 1);
					else
						dradbg(ido, ip, l1, idl1, c, c, c, ch, ch, wa, waoff
								+ iw - 1);
					if (ido == 1)
						na = 1 - na;
					state = L115;
		...
		if (ido != 2) {
			...
			for (k = 0; k < l1; k++) {
				...
				for (i = 2; i < ido; i += 2) {
					...
					ch[t3] = cc[t4] - cc[t5];
					ti2 = cc[t4] + cc[t5];
					ch[t6 - 1] = wa1[waoff + i - 2] * tr2 - wa1[waoff + i - 1]
							* ti2;
					ch[t6] = wa1[waoff + i - 2] * ti2 + wa1[waoff + i - 1]
							* tr2;
				}
				t2 = (t1 += ido) << 1;
			}
	...
	static void dradb3(int ido, int l1, float[] cc, float[] ch, float[] wa,
			int wa1, int wa2) {
	...
	static void dradb4(int ido, int l1, float[] cc, float[] ch, float[] wa,
			int wa1, int wa2, int wa3) {
		...
		if (ido != 2) {
			...
			for (k = 0; k < l1; k++) {
				...
				for (i = 2; i < ido; i += 2) {
					...
					ch[(t8 = t7 + t0) - 1] = wa[wa1 + i - 2] * cr2
							- wa[wa1 + i - 1] * ci2;
					ch[t8] = wa[wa1 + i - 2] * ci2 + wa[wa1 + i - 1] * cr2;
					ch[(t8 += t0) - 1] = wa[wa2 + i - 2] * cr3
							- wa[wa2 + i - 1] * ci3;
					ch[t8] = wa[wa2 + i - 2] * ci3 + wa[wa2 + i - 1] * cr3;
					ch[(t8 += t0) - 1] = wa[wa3 + i - 2] * cr4
							- wa[wa3 + i - 1] * ci4;
			...
			for (j = 1; j < ip; j++) {
				...
				for (k = 0; k < l1; k++) {
					...
					for (i = 2; i < ido; i += 2) {
						idij += 2;
						t3 += 2;
						c1[t3 - 1] = wa[waoff + idij - 1] * ch[t3 - 1]
								- wa[waoff + idij] * ch[t3];
						c1[t3] = wa[waoff + idij - 1] * ch[t3]
								+ wa[waoff + idij] * ch[t3 - 1];
					}
					t2 += ido;
				}
			}
