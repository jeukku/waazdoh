...
class oggpack_buffer {
	...
	static int[] mask = new int[] { 0x00000000, 0x00000001, 0x00000003,
	...
	public boolean vorbis_staticbook_pack(static_codebook c) {
		...
		// pack the codewords. There are two packings; length ordered and length
		...
		if (ordered != 0) {
			// length ordered. We only need to say how many codewords of each
			// length.
			...
			// length random. Again, we don't code the codeword itself, just the
			// length.
			...
			// here.
			// The algorithmic mapping happens as usual, but the unused entry
			// has no codeword.
		...
		default:
			...
			System.out
					.println("error case; we don't have any other map types now");
			return false;
		}
	...
	public int floor1_encode(vorbis_block vb, vorbis_look_floor1 look,
		...
		if (post != null) {
			...
			for (i = 2; i < posts; i++) {
				...
				if (((post[i] & 0x8000) > 0) || (predicted == post[i])) {
					...
					int headroom = (look.quant_q - predicted < predicted ? look.quant_q
							- predicted
							: predicted);

					int val = post[i] - predicted;

					// at this point the 'deviation' value is in the range +/-
					// max
					// range, but the real, unique range can always be mapped to
					// only [0-maxrange). So we want to wrap the deviation into
					// this limited range, but do it in the way that least
					// screws
			...
			for (i = 0, j = 2; i < info.partitions; i++) {
				...
				if (csubbits > 0) {
					...
					look.phrasebits += vorbis_book_encode(
							books[info.class_book[class_local]], cval);
				}
	...
	public void res0_pack(vorbis_info_residue0 info) {
		...
		oggpack_write(info.grouping - 1, 24); // residue vectors to group and
