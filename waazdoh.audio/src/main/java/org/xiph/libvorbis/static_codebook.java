...
public class static_codebook {
	...
	int q_min; // long q_min // packed 32 bit float; quant value 0 maps to
				// minval
	...
	public static_codebook(int _dim, int _entries, int[] _lengthlist,
			int _maptype, int _q_min, int _q_delta, int _q_quant,
			int _q_sequencep, int[] _quantlist,
			encode_aux_nearestmatch _nearest_tree,
			encode_aux_threshmatch _thresh_tree,
			encode_aux_pigeonhole _pigeon_tree, int _allocedp) {
	...
	public static_codebook(static_codebook src) {
		...
		this(src.dim, src.entries, src.lengthlist, src.maptype, src.q_min,
				src.q_delta, src.q_quant, src.q_sequencep, src.quantlist,
				new encode_aux_nearestmatch(src.nearest_tree),
				new encode_aux_threshmatch(src.thresh_tree),
				new encode_aux_pigeonhole(src.pigeon_tree), src.allocedp);
	}
	...
	public int _book_maptype1_quantvals() {
		...
		// long vals
		int vals = new Double(Math.floor(Math.pow(
				new Integer(entries).floatValue(), 1.0f / dim))).intValue();
