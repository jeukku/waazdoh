...
class envelope_filter_state {
	...
	public envelope_filter_state(float[] _ampbuf, int _ampptr, float[] _nearDC,
			float _nearDC_acc, float _nearDC_partialacc, int _nearptr) {
	...
	public envelope_filter_state(envelope_filter_state src) {
		...
		this(src.ampbuf, src.ampptr, src.nearDC, src.nearDC_acc,
				src.nearDC_partialacc, src.nearptr);
	}
