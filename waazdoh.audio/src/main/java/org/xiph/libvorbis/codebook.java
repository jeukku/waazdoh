...
public class codebook {
	...
	// for decode, the below are ordered by bitreversed codeword and only used
	// entries are populated

	float[] valuelist; // list of dim*entries actual entry values
	int[] codelist; // ogg_uint32_t *codelist // list of bitstream codewords for
					// each entry
	...
	private int[] _make_words(int[] l, int n, int sparsecount) {
		...
		for (i = 0; i < n; i++) {
			...
			if (length > 0) {
				...
				// below
				// it (pruning off the imagined tree that may have dangled from
				// it)
				// as well as blocking the use of any nodes directly above for
				// leaves
				...
				// markers were
				// dangling from our just-taken node. Dangle them from our *new*
				// node.
	...
	private float ldexp(double value, long exp) {
		...
		return new Double(value * Math.pow(2, new Long(exp).intValue()))
				.floatValue();
	}
	...
	private float[] _book_unquantize(static_codebook b, int n, int[] sparsemap) {
		...
		if (b.maptype == 1 || b.maptype == 2) {
			...
			// maptype 1 and 2 both use a quantized value vector, but different
			// sizes
