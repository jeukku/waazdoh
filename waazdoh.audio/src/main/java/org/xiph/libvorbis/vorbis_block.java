...
public class vorbis_block {
	...
	// local storage to avoid remallocing; it's up to the mapping to structure
	// it
	...
	public boolean vorbis_analysis_blockout(vorbis_dsp_state v) {
		...
		// we do an envelope search even on a single blocksize; we may still be
		// throwing
		// more bits at impulses, and envelope search handles marking impulses
		// too
		...
		if (bp == -1) {
			...
			if (v.eofflag == 0)
				return false; // not enough data currently to search for a full
								// long block
			v.nW = 0;
		} else {
		...
		centerNext = v.centerW + ci.blocksizes[v.W] / 4 + ci.blocksizes[v.nW]
				/ 4;
		...
		// we have some duplicate memory arrays probably increasing memory
		// footprint
		...
		// vbi.pcmdelay =
		// _vorbis_block_alloc(vb,sizeof(*vbi.pcmdelay)*v.vi.channels);
		vbi.pcmdelay = new float[v.vi.channels][];

		for (i = 0; i < v.vi.channels; i++) {

			// vbi.pcmdelay[i] =
			// _vorbis_block_alloc(vb,(pcmend+beginW)*sizeof(*vbi.pcmdelay[i]));
			vbi.pcmdelay[i] = new float[pcmend + beginW];

			// memcpy( vbi.pcmdelay[i], v.pcm[i],
			// (pcmend+beginW)*sizeof(*vbi.pcmdelay[i]) );
			...
			System.arraycopy(vbi.pcmdelay[i], beginW, pcm[i], 0,
					vbi.pcmdelay[i].length - beginW);
		}
		...
		if (movementW > 0) {
			...
			for (i = 0; i < v.vi.channels; i++) {
				...
				System.arraycopy(v.pcm[i], movementW, v.pcm[i], 0,
						v.pcm_current);
			}
	...
	public boolean vorbis_analysis(ogg_packet op) {
		...
		// figure out what soft mode to use. This allows easier bitrate
		// management
		...
		if (op != null) {
			...
			if (vorbis_bitrate_managed())
				return false; // The app is using a bitmanaged mode... but not
								// using the bitrate management interface.
	...
	static int accumulate_fit(float[] flr, int mdct, int x0, int x1,
	...
	static int inspect_error(int x0, int x1, int y0, int y1, float[] mask,
			int mdct, vorbis_info_floor1 info) {
	...
	public int[] floor1_fit(vorbis_look_floor1 look, int logmdct,
			float[] logmask) {
		...
		// int loneighbor[VIF_POSIT+2]; // sorted index of range list position
		// (+2)
		...
		int[] fit_valueA = new int[VIF_POSIT + 2]; // index by range list
													// position
		int[] fit_valueB = new int[VIF_POSIT + 2]; // index by range list
													// position

		int[] loneighbor = new int[VIF_POSIT + 2]; // sorted index of range list
													// position (+2)
		...
		if (posts == 0) {
			...
			for (i = 0; i < posts - 1; i++)
				nonzero += accumulate_fit(logmask, logmdct,
						look.sorted_index[i], look.sorted_index[i + 1],
						fits[i], n, info);
		}
		...
		if (nonzero > 0) {
			...
			for (i = 2; i < posts; i++) {
				...
				if (memo[ln] != hn) {
					...
					if (ly == -1 || hy == -1) {
						System.out
								.println("ERROR: We want to bound/minimize *local*, not global");
						System.exit(1);
					}
					...
					if (inspect_error(lx, hx, ly, hy, logmask, logmdct, info) > 0) {
						...
						int hy0 = -200;
						int hy1 = -200;
						return_buffer = fit_line(fits, lsortpos, sortpos
								- lsortpos, ly0, ly1);
						ly0 = return_buffer[0];
						ly1 = return_buffer[1];
						return_buffer = fit_line(fits, sortpos, hsortpos
								- sortpos, hy0, hy1);
						hy0 = return_buffer[0];
						hy1 = return_buffer[1];
	...
	static int[] floor1_interpolate_fit(vorbis_look_floor1 look, int[] A,
			int[] B, int del) {
		...
		if (A != null && B != null) {
			...
			for (i = 0; i < posts; i++) {
				output[i] = ((65536 - del) * (A[i] & 0x7fff) + del
						* (B[i] & 0x7fff) + 32768) >> 16;
	...
	private float[][] _vp_quantize_couple_memo(vorbis_info_psy_global g,
			vorbis_look_psy p, vorbis_info_mapping0 vi, float[][] mdct) {
	...
	private static void SORT4(int o, float[] data, int offset, int[] n) {
		...
		if (Math.abs(data[offset + o + 2]) >= Math.abs(data[offset + o + 3]))
			if (Math.abs(data[offset + o + 0]) >= Math
					.abs(data[offset + o + 1]))
				if (Math.abs(data[offset + o + 0]) >= Math.abs(data[offset + o
						+ 2]))
					if (Math.abs(data[offset + o + 1]) >= Math.abs(data[offset
							+ o + 2])) {
						n[o] = o + 0;
						n[o + 1] = o + 1;
						n[o + 2] = o + 2;
						n[o + 3] = o + 3;
					} else if (Math.abs(data[offset + o + 1]) >= Math
							.abs(data[offset + o + 3])) {
						n[o] = o + 0;
						n[o + 1] = o + 2;
						n[o + 2] = o + 1;
				...
				else if (Math.abs(data[offset + o + 0]) >= Math.abs(data[offset
						+ o + 3]))
					if (Math.abs(data[offset + o + 1]) >= Math.abs(data[offset
							+ o + 3])) {
						n[o] = o + 2;
			...
			else if (Math.abs(data[offset + o + 1]) >= Math.abs(data[offset + o
					+ 2]))
				if (Math.abs(data[offset + o + 0]) >= Math.abs(data[offset + o
						+ 2])) {
					n[o] = o + 1;
					...
					n[o + 2] = o + 2;
					n[o + 3] = o + 3;
				} else if (Math.abs(data[offset + o + 0]) >= Math
						.abs(data[offset + o + 3])) {
					n[o] = o + 1;
					n[o + 1] = o + 2;
					n[o + 2] = o + 0;
			...
			if (Math.abs(data[offset + o + 0]) >= Math
					.abs(data[offset + o + 3]))
				if (Math.abs(data[offset + o + 1]) >= Math.abs(data[offset + o
						+ 3])) {
					n[o] = o + 0;
					n[o + 1] = o + 1;
					n[o + 2] = o + 3;
			...
			else if (Math.abs(data[offset + o + 0]) >= Math.abs(data[offset + o
					+ 2]))
				if (Math.abs(data[offset + o + 1]) >= Math.abs(data[offset + o
						+ 2])) {
					n[o] = o + 3;
	...
	static void sortindex_fix8(int[] index, int ioff, float[] data, int offset) {
		...
		k = 4;
		for (i = 0; i < 8; i++)
			index[ioff + offset + i] = n[((k >= 8)
					|| (j < 4)
					&& (Math.abs(data[offset + 0 + n[j]]) >= Math
							.abs(data[offset + 0 + n[k]])) ? j++ : k++)]
					+ offset;
	}
	...
	static void sortindex_fix32(int[] index, int ioff, float[] data, int offset) {
		...
		for (i = j = 0, k = 8; i < 16; i++)
			n[i] = index[ioff
					+ ((k >= 16)
							|| (j < 8)
							&& (Math.abs(data[0 + index[ioff + j]]) >= Math
									.abs(data[0 + index[ioff + k]])) ? j++
							: k++)];
		...
		for (i = j = 0, k = 16; i < 32; i++)
			index[ioff + i] = n[((k >= 32) || (j < 16)
					&& (Math.abs(data[0 + n[j]]) >= Math.abs(data[0 + n[k]])) ? j++
					: k++)];
	}

	static void sortindex_shellsort(int[] index, int ioff, float[] data,
	...
	static void sortindex(int[] index, int ioff, float[] data, int offset,
			int count) {
	...
	private int[][] _vp_quantize_couple_sort(vorbis_look_psy p,
			vorbis_info_mapping0 vi, float[][] mags) {
		...
		if (p.vi.normal_point_p > 0) {
			...
			// int
			// **ret=_vorbis_block_alloc(vb,vi->coupling_steps*sizeof(*ret));
			...
			for (i = 0; i < vi.coupling_steps; i++) {
				...
				for (j = 0; j < n; j += partition) {
					/*
					 * for ( k=0; k < partition; k++ ) { // work[k] =
					 * mags[i]+k+j; work[k] = new float[ mags[i].length - k+j ];
					 * System.arraycopy(mags[i], k+j, work[k], 0, mags[i].length
					 * - k+j ); }
					 * 
					 * // qsort(work,partition,sizeof(*work),apsort);
					 * Arrays.sort( work );
					 * 
					 * // for( k=0; k < partition; k++ ) // ret[i][k+j] =
					 * work[k]-mags[i]; for ( k=0; k < partition; k++)
	...
	private void _vp_noise_normalize_sort(vorbis_look_psy p,
	...
	private void hf_reduction(vorbis_info_psy_global g, vorbis_look_psy p,
			vorbis_info_mapping0 vi, float[][] mdct) {
		...
		for (i = 0; i < vi.coupling_steps; i++) {
			...
			for (j = limit; j < n; j++)
				mdct[i][j] *= (1.0 - de
						* ((float) (j - limit) / (float) (n - limit)));
		}
	}

	private void _vp_remove_floor(vorbis_look_psy p, float[] mdct,
			int[] codedflr, float[] residue, int sliding_lowpass) {
	...
	private void _vp_noise_normalize(vorbis_look_psy p, float[] in, int out,
			int[] sortedindex) {
	...
	-0.009935f, -0.011245f, -0.012726f, -0.014397f, -0.016282f, -0.018407f,
			-0.020800f, -0.023494f, -0.026522f, -0.029923f, -0.033737f,
			-0.038010f, -0.042787f, -0.048121f, -0.054064f, -0.060671f,
			-0.068000f, -0.076109f, -0.085054f, -0.094892f, -0.105675f,
			-0.117451f, -0.130260f, -0.144134f, -0.159093f, -0.175146f,
			-0.192286f, -0.210490f, -0.229718f, -0.249913f, -0.271001f,
			-0.292893f };

	static float[] precomputed_couple_point(float premag, int floorA,
			int floorB, float mag, float ang) {

		int test = ((floorA > floorB) ? 1 : 0) - 1;
		int offset = 31 - Math.abs(floorA - floorB);
		float floormag = hypot_lookup[(((offset < 0) ? 1 : 0) - 1) & offset] + 1.f;

		floormag *= FLOOR1_fromdB_INV_LOOKUP[(floorB & test)
				| (floorA & (~test))];
	...
	private void _vp_couple(int blobno, vorbis_info_psy_global g,
			vorbis_look_psy p, vorbis_info_mapping0 vi, float[][] res,
			float[][] mag_memo, int[][] mag_sort, int[][] ifloor,
		...
		for (i = 0; i < vi.coupling_steps; i++) {
			...
			if ((nonzero[vi.coupling_mag[i]] > 0)
				...
				int partition = ((p.vi.normal_point_p > 0) ? p.vi.normal_partition
						: p.n);
				...
				for (j = 0; j < p.n; j += partition) {
					...
					for (k = 0; k < partition; k++) {
						...
						if (l < sliding_lowpass) {
							...
							if ((l >= limit && Math.abs(rM[l]) < postpoint && Math
									.abs(rA[l]) < postpoint)
									|| (Math.abs(rM[l]) < prepoint && Math
											.abs(rA[l]) < prepoint)) {

								float[] return_buffer = precomputed_couple_point(
										mag_memo[i][l], floorM[l], floorA[l],
										rM[qM + l], rA[qA + l]);
								...
								float[] return_buffer = couple_lossless(rM[l],
										rA[l], rM[qM + l], rA[qA + l]);
								rM[qM + l] = return_buffer[0];
								rA[qA + l] = return_buffer[1];
							}
						} else {
	...
	private int[][] _2class(vorbis_look_residue0 look, float[][] in,
			int in_offset, int ch) {
		...
		// partword[0] =
		// _vorbis_block_alloc(vb,n*ch/samples_per_partition*sizeof(*partword[0]));
	...
	private int[][] res2_class(vorbis_look_residue0 vl, float[][] in,
			int in_offset, int[] nonzero, int ch) {
	...
	private int _encodepart(oggpack_buffer opb_local, float[] vec, int vec_off,
			int n, codebook book, int[] acc) {
	...
	private boolean _01forward(oggpack_buffer opb_local,
			vorbis_look_residue0 look, float[] in, int ch, int[][] partword) {
				...
				if (s == 0) {
					...
					for (j = 0; j < ch; j++) {
						...
						if (val < look.phrasebook.entries)
							look.phrasebits += opb_local.vorbis_book_encode(
									look.phrasebook, val);
						else
							System.out
									.println("! - val < look.phrasebook.entries - !");
					}
				}
					...
					for (j = 0; j < ch; j++) {
						...
						if ((info.secondstages[partword[j][i]] & (1 << s)) > 0) {
							...
							if (statebook != null) {
								...
								ret = _encodepart(opb_local, in, j + offset,
	...
	// res2 is slightly more different; all the channels are interleaved into a
	// single vector and encoded.
	private boolean res2_forward(oggpack_buffer opb_local,
			vorbis_look_residue0 vl, float[][] in, int in_offset,
			float[][] out, int[] nonzero, int ch, int[][] partword) {
	...
	public void mapping0_forward() {
		...
		for (i = 0; i < vi.channels; i++) {
			...
			// TODO - trace where the logfft (pcm) data gets slightly thrown
			// from C
			// causes flr errors later call to accumulate_fit( logmask, logmdct,
			// look.sorted_index[i], look.sorted_index[i+1], fits[i], n, info );
			// sets off the lsfit_acc data when flr[mdct+i]+info.twofitatten >=
			// flr[i]
			...
			window._vorbis_apply_window(pcm[i], b.window, ci.blocksizes, lW, W,
					nW);
			...
			for (j = 1; j < n - 1; j += 2) {
				float temp = pcm[i][j] * pcm[i][j] + pcm[i][j + 1]
						* pcm[i][j + 1];
		...
		for (i = 0; i < vi.channels; i++) {
			...
			// b.psy[ psy_look_offset ]._vp_noisemask( logmdct, noise); // noise
			// does not have by-frequency offset bias applied yet
			b.psy[psy_look_offset]._vp_noisemask(logfft, logmdct, noise); // noise
																			// does
																			// not
																			// have
																			// by-frequency
			...
			b.psy[psy_look_offset]._vp_tonemask(logfft, tone, global_ampmax,
					local_ampmax[i]);
			...
			// b.psy[ psy_look_offset ]._vp_offset_and_mix( noise, tone, 1,
			// logmask, mdct, logmdct );
			b.psy[psy_look_offset]._vp_offset_and_mix(noise, tone, 1, logfft,
					mdct, logmdct);
			...
			if (ci.floor_type[info.floorsubmap[submap]] != 1) {
				System.out
						.println("Error vorbis_block::mapping0_forward() - floor_type != floor1");
				return;
			}
			...
			floor_posts[i][PACKETBLOBS / 2] = floor1_fit(
					b.flr[info.floorsubmap[submap]], logmdct, logfft);

			// are we managing bitrate? If so, perform two more fits for later
			// rate tweaking (fits represent hi/lo)
			if (vorbis_bitrate_managed()
					&& (floor_posts[i][PACKETBLOBS / 2] != null)) {

				// higher rate by way of lower noise curve
				b.psy[psy_look_offset]._vp_offset_and_mix(noise, tone, 2,
						logfft, mdct, logmdct);
				...
				b.psy[psy_look_offset]._vp_offset_and_mix(noise, tone, 0,
						logfft, mdct, logmdct);

				// floor_posts[i][0] = floor1_fit( vb,
				// b.flr[info.floorsubmap[submap]], logmdct, logmask );
				floor_posts[i][0] = floor1_fit(b.flr[info.floorsubmap[submap]],
						logmdct, logfft);
				...
				for (k = 1; k < PACKETBLOBS / 2; k++) {
					floor_posts[i][k] = floor1_interpolate_fit(
							b.flr[info.floorsubmap[submap]], floor_posts[i][0],
							floor_posts[i][PACKETBLOBS / 2], k * 65536
									/ (PACKETBLOBS / 2));
				}
		...
		// the next phases are performed once for vbr-only and PACKETBLOB times
		// for bitrate managed modes.
		...
		if (info.coupling_steps > 0) {
			...
			mag_memo = _vp_quantize_couple_memo(ci.psy_g_param,
					b.psy[psy_look_offset], info, gmdct);
			mag_sort = _vp_quantize_couple_sort(b.psy[psy_look_offset], info,
					mag_memo);
		...
		if (b.psy[psy_look_offset].vi.normal_channel_p > 0) {
			...
			for (i = 0; i < vi.channels; i++) {
				...
				_vp_noise_normalize_sort(b.psy[psy_look_offset], mdct,
						sortindex[i]);
			}
		}

		for (k = (vorbis_bitrate_managed() ? 0 : PACKETBLOBS / 2); k <= (vorbis_bitrate_managed() ? PACKETBLOBS - 1
				: PACKETBLOBS / 2); k++) {
			...
			for (i = 0; i < vi.channels; i++) {
				...
				nonzero[i] = opb_local.floor1_encode(this,
						b.flr[info.floorsubmap[submap]], floor_posts[i][k],
						ilogmask);

				_vp_remove_floor(b.psy[psy_look_offset], mdct, ilogmask, res,
						ci.psy_g_param.sliding_lowpass[W][k]);

				_vp_noise_normalize(b.psy[psy_look_offset], res, n / 2,
						sortindex[i]);
			}
			...
			// quantize/couple
			// incomplete implementation that assumes the tree is all depth one,
			// or no tree at all

			if (info.coupling_steps > 0) {
				_vp_couple(k, ci.psy_g_param, b.psy[psy_look_offset], info,
						pcm, mag_memo, mag_sort, ilogmaskch, nonzero,
						ci.psy_g_param.sliding_lowpass[W][k]);
			}
			...
			for (i = 0; i < info.submaps; i++) {
				...
				// ch_in_bundle );
				classifications = res2_class(b.residue[resnum], couple_bundle,
						couple_bundle_offset, zerobundle, ch_in_bundle);

				// _residue_P[ci.residue_type[resnum]].forward( vb,
				// b.residue[resnum], couple_bundle, NULL, zerobundle,
				// ch_in_bundle, classifications );
				res2_forward(opb_local, b.residue[resnum], couple_bundle,
						couple_bundle_offset, null, zerobundle, ch_in_bundle,
						classifications);
			}
	...
	public boolean vorbis_bitrate_addblock() {
		...
		int desired_fill = new Float(bi.reservoir_bits * bi.reservoir_bias)
				.intValue(); // long
		...
		if (bm.avg_bitsper > 0) {
			...
			slew = new Double(Math.rint(choice - bm.avgfloat) / samples
					* vi.rate).floatValue();
			if (slew < -slewlimit)
				slew = -slewlimit;
			if (slew > slewlimit)
				slew = slewlimit;
			choice = new Float(Math.rint(bm.avgfloat += slew / vi.rate
					* samples)).intValue();
			this_bits = vbi.packetblob[choice].oggpack_bytes() * 8;
		}
