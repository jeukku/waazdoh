...
class bitrate_manager_state {
	...
	public bitrate_manager_state(vorbis_info vi) {
		...
		if (bi.reservoir_bits > 0) {
			...
			avg_bitsper = new Double(Math.rint(1. * bi.avg_rate * halfsamples
					/ ratesamples)).intValue();
			min_bitsper = new Double(Math.rint(1. * bi.min_rate * halfsamples
					/ ratesamples)).intValue();
			max_bitsper = new Double(Math.rint(1. * bi.max_rate * halfsamples
					/ ratesamples)).intValue();
			...
			{
				int desired_fill = new Float(bi.reservoir_bits
						* bi.reservoir_bias).intValue(); // long
