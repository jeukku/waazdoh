...
public class ve_setup_data_template {
	...
	float[] psy_noise_compand_short_mapping; // double
												// *psy_noise_compand_short_mapping
	float[] psy_noise_compand_long_mapping; // double
	...
	public ve_setup_data_template(int _mappings, float[] _rate_mapping,
			float[] _quality_mapping, int _coupling_restriction,
			int _samplerate_min_restriction, int _samplerate_max_restriction,
			int[] _blocksize_short, int[] _blocksize_long,
			att3[] _psy_tone_masteratt, int[] _psy_tone_0dB,
			int[] _psy_tone_dBsuppress, vp_adjblock[] _psy_tone_adj_impulse,
			vp_adjblock[] _psy_tone_adj_long,
			vp_adjblock[] _psy_tone_adj_other, noiseguard[] _psy_noiseguards,
			noise3[] _psy_noise_bias_impulse, noise3[] _psy_noise_bias_padding,
			noise3[] _psy_noise_bias_trans, noise3[] _psy_noise_bias_long,
			int[] _psy_noise_dBsuppress, compandblock[] _psy_noise_compand,
			float[] _psy_noise_compand_short_mapping,
			float[] _psy_noise_compand_long_mapping,
			int[][] _psy_noise_normal_start,
			int[][] _psy_noise_normal_partition,
			float[] _psy_noise_normal_thresh, int[] _psy_ath_float,
			int[] _psy_ath_abs, float[] _psy_lowpass,
			vorbis_info_psy_global[] _global_params, float[] _global_mapping,
			adj_stereo[] _stereo_modes, static_codebook[][] _floor_books,
			vorbis_info_floor1[] _floor_params, int[] _floor_short_mapping,
			int[] _floor_long_mapping, vorbis_mapping_template[] _maps) {
		mappings = _mappings;
