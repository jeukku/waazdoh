...
public class vorbis_dsp_state {
	...
	private vorbis_look_residue0 res0_look(vorbis_info_residue0 info) {
		...
		look.partvals = new Double(Math.rint(Math.pow(look.parts, dim)))
				.intValue();
	...
	private boolean _vds_shared_init(boolean encp) {
		...
		if (encp) { // encode/decode differ here
			...
			for (i = 0; i < ci.psys; i++) {
				...
				backend_state.psy[i]._vp_psy_init(ci.psy_param[i],
						ci.psy_g_param,
	...
	public boolean vorbis_analysis_headerout(vorbis_comment vc, ogg_packet op,
		...
		System.arraycopy(opb.buffer, 0, backend_state.header, 0,
				opb.oggpack_bytes());
		...
		System.arraycopy(opb.buffer, 0, backend_state.header1, 0,
				opb.oggpack_bytes());
		...
		System.arraycopy(opb.buffer, 0, backend_state.header2, 0,
				opb.oggpack_bytes());
	...
	public float[][] vorbis_analysis_buffer(int vals) {
		...
		if (pcm_current + vals >= pcm_storage) {
			...
			for (i = 0; i < vi.channels; i += 2) {
				// pcm[i] = _ogg_realloc( v->pcm[i],
				// v->pcm_storage*sizeof(*v->pcm[i]) );
		...
		if (vals <= 0) {
			...
			for (i = 0; i < vi.channels; i++) {
				...
				if (eofflag > order * 2) {
					...
					vorbis_lpc_predict(lpc, pcm[i], eofflag - order, order,
							eofflag, pcm_current - eofflag);
			...
			if ((preextrapolate <= 0)
					&& pcm_current - centerW > ci.blocksizes[1])
				_preextrapolate_helper();
		}
		return true;
	}
	...
	public void _preextrapolate_helper() {
		...
		if (pcm_current - centerW > order * 2) { // safety
			...
			for (i = 0; i < vi.channels; i++) {
				...
				// run the predictor filter
				vorbis_lpc_predict(lpc, work, pcm_current - centerW - order,
						order, pcm_current - centerW, centerW);
	...
	public float vorbis_lpc_from_data(float[] data, int offset1, float[] lpci,
			int n, int m) {
	...
	public void vorbis_lpc_predict(float[] coeff, float[] prime, int offset1,
			int m, int offset2, int n) {
	...
	public int _ve_envelope_search() {
		...
		for (j = first; j < last; j++) {
			...
			for (i = 0; i < ve.ch; i++) {
				...
				// j );
				ret |= ve._ve_amp(gi, pcm[i], ve.searchstep * (j), ve.band,
						ve.filter, i * VE_BANDS, j);
			}
		...
		int testW = centerW_local + ci.blocksizes[W] / 4 + ci.blocksizes[1] / 2
				+ ci.blocksizes[0] / 4;
