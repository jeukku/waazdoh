...
/*********************************************************************
 * Encoding using a VBR quality mode. The usable range is -.1 (lowest quality,
 * smallest file) to 1. (highest quality, largest file). Example quality mode
 * .4: 44kHz stereo coupled, roughly 128kbps VBR
 * 
 * ret = vorbis_encode_init_vbr(&vi,2,44100,.4);
 * 
 * ---------------------------------------------------------------------
 * 
 * Encoding using an average bitrate mode (ABR). example: 44kHz stereo coupled,
 * average 128kbps VBR
 * 
 ...
 * ---------------------------------------------------------------------
 * 
 * Encode using a quality mode, but select that quality mode by asking for an
 * approximate bitrate. This is not ABR, it is true VBR, but selected using the
 * bitrate interface, and then turning bitrate management off:
 * 
...
public class vorbisenc {
	...
	public boolean vorbis_encode_setup_vbr(int channels, int rate, float quality) {
		...
		if (!get_setup_template(channels, rate, quality, 0)) {
			System.out.println("Unable to find setup template - " + channels
					+ " channels, " + rate + " rate, " + quality + " quality");
			return false;
		}
	...
	public boolean vorbis_encode_init_vbr(vorbis_info _vi, int channels,
			int rate, float base_quality) {
	...
	public boolean vorbis_encode_setup_managed(vorbis_info _vi, int channels,
			int rate, int max_bitrate, int nominal_bitrate, int min_bitrate) {
		...
		if (nominal_bitrate <= 0.) {
			if (max_bitrate > 0.) {
				if (min_bitrate > 0.)
					nominal_bitrate = new Double(
							(max_bitrate + min_bitrate) * .5).intValue();
				else
		...
		if (!get_setup_template(channels, rate, nominal_bitrate, 1)) {
			System.out.println("Unable to find setup template - " + channels
					+ " channels, " + rate + " rate, " + nominal_bitrate
					+ " nominal_bitrate");
			return false;
		}
	...
	public boolean vorbis_encode_init(vorbis_info _vi, int channels, int rate,
			int max_bitrate, int nominal_bitrate, int min_bitrate) {

		if (_vi == null)
			return false;

		if (!vorbis_encode_setup_managed(_vi, channels, rate, max_bitrate,
				nominal_bitrate, min_bitrate))
			return false;
	...
	public Object vorbis_encode_ctl(vorbis_info _vi, int number, Object arg) {
		 ...
		 * ai.bitrate_av_window_center=1.; ai.bitrate_hard_min=hi.bitrate_min;
		 * ai.bitrate_hard_max=hi.bitrate_max; ai.bitrate_av_lo=hi.bitrate_av;
		 * ai.bitrate_av_hi=hi.bitrate_av; } return true;
		 * 
		 * // now deprecated ***************** case OV_ECTL_RATEMANAGE_SET: {
		 * 
		 * ovectl_ratemanage_arg ai= ( ovectl_ratemanage_arg )arg;
		 * 
		 * if ( ai == null ) { hi.managed=0; } else {
		 * hi.managed=ai.management_active; vorbis_encode_ctl( vi,
		 * OV_ECTL_RATEMANAGE_AVG, arg ); vorbis_encode_ctl( vi,
		 * OV_ECTL_RATEMANAGE_HARD, arg ); } } return true;
		 * 
		 * // now deprecated ***************** case OV_ECTL_RATEMANAGE_AVG: {
		 * 
		 * ovectl_ratemanage_arg ai= ( ovectl_ratemanage_arg )arg;
		 * 
		 * if (ai == null) { hi.bitrate_av=0; } else {
		 * hi.bitrate_av=(ai.bitrate_av_lo+ai.bitrate_av_hi)*.5; } } return
		 * true;
		 * 
		 * // now deprecated ***************** case OV_ECTL_RATEMANAGE_HARD: {
		 * 
		 * ovectl_ratemanage_arg ai= ( ovectl_ratemanage_arg )arg;
		 * 
		 * if (ai == null ) { hi.bitrate_min=0; hi.bitrate_max=0; } else {
		 * hi.bitrate_min=ai.bitrate_hard_min;
		 * hi.bitrate_max=ai.bitrate_hard_max;
		 * hi.bitrate_reservoir=ai.bitrate_hard_window
		 * *(hi.bitrate_max+hi.bitrate_min)*.5; } if ( hi.bitrate_reservoir <
		 * 128.f ) hi.bitrate_reservoir=128; } return true;
		 */
	...
	private boolean get_setup_template(int ch, int srate, float req,
			int q_or_bitrate) {
		...
		while (setup_list[i] != null) {
			...
			if (setup_list[i].coupling_restriction == -1
				...
				if (srate >= setup_list[i].samplerate_min_restriction
					...
					// fall within this templates modes?
					if (req < map[0]) {
						++i;
						continue;
					}
	...
	private boolean vorbis_encode_setup_setting(int channels, int rate) {
		...
		hi.lowpass_kHz = setup.psy_lowpass[is] * (1.0f - ds)
				+ setup.psy_lowpass[is + 1] * ds;

		hi.ath_floating_dB = setup.psy_ath_float[is] * (1.0f - ds)
				+ setup.psy_ath_float[is + 1] * ds;
		hi.ath_absolute_dB = setup.psy_ath_abs[is] * (1.0f - ds)
		...
		for (i = 0; i < 4; i++)
			hi.block[i] = new highlevel_byblocktype(hi.base_setting,
					hi.base_setting, hi.base_setting, hi.base_setting);
	...
	public boolean vorbis_encode_setup_init() { // the final setup call
		...
		// too low/high an ATH floater is nonsensical, but doesn't break
		// anything
		...
		if (setup == null) {
			System.out
					.println("vorbis_info.codec_setup.highlevel_encode_setup.ve_setup_data_template doesnt exist");
			return false;
		}
		...
		// blocks are the same length, we set long_block_p and unset
		// short_block_p

		vorbis_encode_blocksize_setup(hi.base_setting, setup.blocksize_short,
				setup.blocksize_long);
		if (ci.blocksizes[0] == ci.blocksizes[1])
			singleblock = 1;

		// floor setup; choose proper floor params. Allocated on the floor stack
		// in order; if we alloc only long floor, it's 0
		vorbis_encode_floor_setup(hi.short_setting, 0, setup.floor_books,
				setup.floor_params, setup.floor_short_mapping);
		if (singleblock == 0)
			vorbis_encode_floor_setup(hi.long_setting, 1, setup.floor_books,
					setup.floor_params, setup.floor_long_mapping);
		...
		vorbis_encode_psyset_setup(hi.short_setting,
		...
		if (singleblock == 0) {
			vorbis_encode_psyset_setup(hi.long_setting,
		...
		vorbis_encode_compand_setup(hi.block[i0].noise_compand_setting, 0,
				setup.psy_noise_compand, setup.psy_noise_compand_short_mapping);
		...
		vorbis_encode_noisebias_setup(hi.block[i0].noise_bias_setting, 0,
				setup.psy_noise_dBsuppress, setup.psy_noise_bias_impulse,
				setup.psy_noiseguards, userbias);
		vorbis_encode_noisebias_setup(hi.block[1].noise_bias_setting, 1,
				setup.psy_noise_dBsuppress, setup.psy_noise_bias_padding,
				setup.psy_noiseguards, 0.0f);
		if (singleblock == 0) {
			vorbis_encode_noisebias_setup(hi.block[2].noise_bias_setting, 2,
					setup.psy_noise_dBsuppress, setup.psy_noise_bias_trans,
					setup.psy_noiseguards, 0.0f);
			vorbis_encode_noisebias_setup(hi.block[3].noise_bias_setting, 3,
					setup.psy_noise_dBsuppress, setup.psy_noise_bias_long,
					setup.psy_noiseguards, 0.0f);
		}
		...
		if (hi.bitrate_av > 0)
			...
			vi.bitrate_nominal = new Float(setting_to_approx_bitrate())
					.intValue();
		...
		if (hi.managed > 0) {
			...
			ci.bi = new bitrate_manager_info(hi.bitrate_av, hi.bitrate_min,
					hi.bitrate_max, hi.bitrate_reservoir,
					hi.bitrate_reservoir_bias, hi.bitrate_av_damp);
		}

		return true;
	}

	private void vorbis_encode_blocksize_setup(float s, int[] shortb,
			int[] longb) {
	...
	private void vorbis_encode_floor_setup(float s, int block,
			static_codebook[][] books, vorbis_info_floor1[] in, int[] x) {
	...
	private void vorbis_encode_global_psych_setup(float s,
			vorbis_info_psy_global[] in, float[] x) {
		...
		ci.psy_g_param = new vorbis_info_psy_global(
				in[new Float(x[is]).intValue()]);
		...
		for (i = 0; i < 4; i++) {
			g.preecho_thresh[i] = new Double(in[is].preecho_thresh[i]
					* (1. - ds) + in[is + 1].preecho_thresh[i] * ds)
					.floatValue();
			g.postecho_thresh[i] = new Double(in[is].postecho_thresh[i]
					* (1. - ds) + in[is + 1].postecho_thresh[i] * ds)
					.floatValue();
		}
		g.ampmax_att_per_sec = ci.hi.amplitude_track_dBpersec;
	}

	private void vorbis_encode_global_stereo(highlevel_encode_setup hi,
			adj_stereo[] p) {
		...
		if (p != null) {
			...
			if (hi.managed != 0) {
				...
				for (i = 0; i < PACKETBLOBS; i++) {
					float kHz = new Double(p[is].kHz[i] * (1. - ds)
							+ p[is + 1].kHz[i] * ds).floatValue();
					g.coupling_pointlimit[0][i] = new Double(kHz * 1000.
							/ vi.rate * ci.blocksizes[0]).intValue();
					g.coupling_pointlimit[1][i] = new Double(kHz * 1000.
							/ vi.rate * ci.blocksizes[1]).intValue();
					g.coupling_pkHz[i] = new Float(kHz).intValue();

					kHz = new Double(p[is].lowpasskHz[i] * (1. - ds)
							+ p[is + 1].lowpasskHz[i] * ds).floatValue();
					g.sliding_lowpass[0][i] = new Double(kHz * 1000. / vi.rate
							* ci.blocksizes[0]).intValue();
					g.sliding_lowpass[1][i] = new Double(kHz * 1000. / vi.rate
							* ci.blocksizes[1]).intValue();
				}
			} else {
				float kHz = new Double(p[is].kHz[PACKETBLOBS / 2] * (1. - ds)
						+ p[is + 1].kHz[PACKETBLOBS / 2] * ds).floatValue();
				for (i = 0; i < PACKETBLOBS; i++) {
					g.coupling_pointlimit[0][i] = new Double(kHz * 1000.
							/ vi.rate * ci.blocksizes[0]).intValue();
					g.coupling_pointlimit[1][i] = new Double(kHz * 1000.
				...
				kHz = new Double(p[is].lowpasskHz[PACKETBLOBS / 2] * (1. - ds)
	...
	private void vorbis_encode_psyset_setup(float s, int[] nn_start,
			int[] nn_partition, float[] nn_thresh, int block) {
		...
		ci.psy_param[block] = new vorbis_info_psy(
				...
				new float[] { 0.f, 0.f, 0.f }, 0.f, 0.f, -40.f,
				new float[] { 0.f },

				// noisemaskp,supp, low/high window, low/hi guard, minimum
				1, -0.f, .5f, .5f, 0, 0, 0, new float[][] { { -1 }, { -1 },
						{ -1 } }, new float[] { -1 }, 105.f, 0, 0, -1, -1, 0.0f);
	...
	private void vorbis_encode_tonemask_setup(float s, int block, att3[] att,
			int[] max, vp_adjblock[] in) {
		...
		p.tone_masteratt[0] = new Double(att[is].att[0] * (1. - ds)
				+ att[is + 1].att[0] * ds).floatValue();
		p.tone_masteratt[1] = new Double(att[is].att[1] * (1. - ds)
				+ att[is + 1].att[1] * ds).floatValue();
		p.tone_masteratt[2] = new Double(att[is].att[2] * (1. - ds)
				+ att[is + 1].att[2] * ds).floatValue();
		p.tone_centerboost = new Double(att[is].boost * (1. - ds)
				+ att[is + 1].boost * ds).floatValue();
		p.tone_decay = new Double(att[is].decay * (1. - ds) + att[is + 1].decay
				* ds).floatValue();

		p.max_curve_dB = new Double(max[is] * (1. - ds) + max[is + 1] * ds)
				.floatValue();

		for (i = 0; i < P_BANDS; i++)
			p.toneatt[i] = new Double(in[is].block[i] * (1. - ds)
					+ in[is + 1].block[i] * ds).floatValue();
	}

	private void vorbis_encode_compand_setup(float s, int block,
			compandblock[] in, float[] x) {
		...
		for (i = 0; i < NOISE_COMPAND_LEVELS; i++)
			p.noisecompand[i] = new Double(in[is].data[i] * (1. - ds)
					+ in[is + 1].data[i] * ds).floatValue();
	}
	...
	private void vorbis_encode_peak_setup(float s, int block, int[] suppress) {
		...
		p.tone_abs_limit = new Double(suppress[is] * (1. - ds)
				+ suppress[is + 1] * ds).floatValue();
	}

	private void vorbis_encode_noisebias_setup(float s, int block,
			int[] suppress, noise3[] in, noiseguard[] guard, float userbias) {
		...
		p.noisemaxsupp = new Double(suppress[is] * (1. - ds) + suppress[is + 1]
				* ds).floatValue();
		...
		for (j = 0; j < P_NOISECURVES; j++)
			for (i = 0; i < P_BANDS; i++)
				p.noiseoff[j][i] = new Double(in[is].data[j][i] * (1. - ds)
						+ in[is + 1].data[j][i] * ds).floatValue();
	...
	private void vorbis_encode_residue_setup(int number, int block,
		...
		// lowpass setup/pointlimit
		{
			...
			// the
			// encoding structure, only the samples used to fit the floor
			// approximation
			...
			if (ci.residue_type[block] == 2)
				r.end = new Double((freq / nyq * blocksize * 2) / r.grouping
						+ .9).intValue()
						* r.grouping; // round up only if we're well past
			else
				r.end = new Double((freq / nyq * blocksize) / r.grouping + .9)
						.intValue() * r.grouping; // round up only if we're well
													// past
		}
	}

	// we assume two maps in this encoder
	private void vorbis_encode_map_n_res_setup(float s,
		...
		vorbis_info_mode[] mode = { new vorbis_info_mode(0, 0, 0, 0),
				new vorbis_info_mode(1, 0, 0, 1) }; // _mode_template
		...
		for (i = 0; i < modes; i++) {
			...
			for (j = 0; j < map[i].submaps; j++)
				vorbis_encode_residue_setup(map[i].residuesubmap[j], i,
