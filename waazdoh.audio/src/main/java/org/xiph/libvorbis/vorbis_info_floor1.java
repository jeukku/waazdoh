...
public class vorbis_info_floor1 {
	...
	public vorbis_info_floor1(int _partitions, int[] _partitionclass,
			int[] _class_dim, int[] _class_subs, int[] _class_book,
			int[][] _class_subbook, int _mult, int[] _postlist, float _maxover,
			float _maxunder, float _maxerr, float _twofitweight,
			float _twofitatten, int _n) {

		partitions = _partitions;

		partitionclass = new int[VIF_PARTS];
		System.arraycopy(_partitionclass, 0, partitionclass, 0,
				_partitionclass.length);
		...
		for (int i = 0; i < _class_subbook.length; i++)
			System.arraycopy(_class_subbook[i], 0, class_subbook[i], 0,
					_class_subbook[i].length);
	...
	public vorbis_info_floor1(vorbis_info_floor1 src) {
		...
		this(src.partitions, src.partitionclass, src.class_dim, src.class_subs,
				src.class_book, src.class_subbook, src.mult, src.postlist,
				src.maxover, src.maxunder, src.maxerr, src.twofitweight,
				src.twofitatten, src.n);
	}
}