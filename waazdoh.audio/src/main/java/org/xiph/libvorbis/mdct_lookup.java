...
class mdct_lookup {
	...
	public mdct_lookup(int _n, int _log2n, float[] _trig, int[] _bitrev,
			float _scale) {
	...
	public void mdct_init(int _n) {
		...
		n = _n;
		log2n = new Double(Math.rint(Math.log(new Integer(n).floatValue())
				/ Math.log(2.f))).intValue();
		...
		// trig lookups...
		for (i = 0; i < n / 4; i++) {
			trig[i * 2] = new Double(Math.cos((M_PI / n) * (4 * i)))
					.floatValue();
			trig[i * 2 + 1] = new Double(-Math.sin((M_PI / n) * (4 * i)))
					.floatValue();
			trig[n2 + i * 2] = new Double(Math.cos((M_PI / (2 * n))
					* (2 * i + 1))).floatValue();
			trig[n2 + i * 2 + 1] = new Double(Math.sin((M_PI / (2 * n))
					* (2 * i + 1))).floatValue();
		}

		for (i = 0; i < n / 8; i++) {
			trig[n + i * 2] = new Double(
					Math.cos((M_PI / n) * (4 * i + 2)) * .5).floatValue();
			trig[n + i * 2 + 1] = new Double(
					-Math.sin((M_PI / n) * (4 * i + 2)) * .5).floatValue();
		}
	...
	private void mdct_butterfly_generic(float[] T, float[] x, int off,
	...
	private void mdct_butterflies(float[] x, int off, int points) {
		...
		for (i = 1; --stages > 0; i++) {
			for (j = 0; j < (1 << i); j++)
				mdct_butterfly_generic(T, x, off + ((points >> i) * j),
						points >> i, 4 << i);
		}
	...
	public void mdct_forward_offset(float[] in, int offset, float[] out,
			int out_offset) {
		...
		for (i = 0; i < n4; i++) {
			...
			out[out_offset + x0 + 0] = ((w[w1 + 0] * T[t + 1] - w[w1 + 1]
					* T[t + 0]) * scale);
			w1 += 2;
			t += 2;
		}
