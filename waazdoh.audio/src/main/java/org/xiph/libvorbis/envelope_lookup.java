...
class envelope_lookup {
	...
	public envelope_lookup(int _ch, int _winlength, int _searchstep,
			float _minenergy, mdct_lookup _mdct, float[] _mdct_win,
			envelope_band[] _band, envelope_filter_state[] _filter,
			int _stretch, int[] _mark, int _storage, int _current,
			int _curmark, int _cursor) {
	...
	public envelope_lookup(envelope_lookup src) {
		...
		this(src.ch, src.winlength, src.searchstep, src.minenergy, src.mdct,
				src.mdct_win, src.band, src.filter, src.stretch, src.mark,
				src.storage, src.current, src.curmark, src.cursor);
	}
	...
	public envelope_lookup(vorbis_info _vi) {
		...
		for (int i = 0; i < n; i++) {
			mdct_win[i] = new Double(Math.sin(i / (n - 1.) * M_PI))
					.floatValue();
		...
		for (int j = 0; j < VE_BANDS; j++) {
			...
			for (int i = 0; i < n; i++) {
				band[j].window[i] = new Double(Math.sin((i + .5) / n * M_PI))
						.floatValue();
	...
	public int _ve_amp(vorbis_info_psy_global gi, float[] data, int offset1,
		...
		/*
		 * we want to have a 'minimum bar' for energy, else we're just basing
		 * blocks on quantization noise that outweighs the signal itself (for
		 * low power signals)
		 */
		...
		/*
		 * _analysis_output_always("lpcm",seq2,data,n,0,0,totalshift+pos*ve->
		 * searchstep);
		 */
		 ...
		 */
		float temp = vec[0] * vec[0] + .7f * vec[1] * vec[1] + .2f * vec[2]
				* vec[2];
		...
		if (ptr == 0) {
			decay = filters[offset2].nearDC_acc = filters[offset2].nearDC_partialacc
					+ temp;
		...
		/*
		 * perform spreading and limiting, also smooth the spectrum. yes, the
		 * MDCT results in all real coefficients, but it still *behaves* like
	...
	public void _ve_envelope_shift(int shift) {
		...
		int smallsize = current / searchstep + VE_POST; // adjust for placing
														// marks ahead of
														// ve->current
