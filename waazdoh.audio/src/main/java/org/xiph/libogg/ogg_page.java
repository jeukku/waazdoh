...
public class ogg_page {
	...
	private static int crc_entry(int index) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if ((r & 0x80000000) != 0) {
				r = (r << 1) ^ 0x04c11db7;
				/*
				 * The same as the ethernet generator polynomial, although we
				 * use an unreflected alg and an init/final of 0, not 0xffffffff
				 */
			} else {
				r <<= 1;
			}
	...
	public void ogg_page_checksum_set() {
		...
		for (i = 0; i < header_len; i++) {
			crc_reg = (crc_reg << 8)
					^ crc_lookup[((crc_reg >>> 24) & 0xff) ^ (header[i] & 0xff)];
		}
		for (i = 0; i < body_len; i++) {
			crc_reg = (crc_reg << 8)
					^ crc_lookup[((crc_reg >>> 24) & 0xff) ^ (body[i] & 0xff)];
		}
