...
public class ogg_stream_state {
	...
	long[] granule_vals; // ogg_int64_t //granulepos values for headers. Not
							// compact this way, but it is simple coupled to the
							// lacing fifo
	...
	byte[] header; // unsigned char header[282]; //working space for header
					// encode
	int header_fill;

	int e_o_s; // set when we have buffered the last packet in the logical
				// bitstream
	int b_o_s; // set after we've written the initial page of a logical
				// bitstream
	int serialno; // long
	int pageno; // long
	long packetno; // ogg_int64_t // sequence number for decode; the framing
					// knows where there's a hole in the data, but we need
					// coupling so
	...
	// Helpers for ogg_stream_encode; this keeps the structure and what's
	// happening fairly clear
	...
	public void _os_body_expand(int needed) {
		...
		if (body_storage <= body_fill + needed) {
			...
			// body_data =
			// _ogg_realloc(os->body_data,os->body_storage*sizeof(*os->body_data));
	...
	public void _os_lacing_expand(int needed) {
		...
		if (lacing_storage <= lacing_fill + needed) {
			...
			// lacing_vals =
			// _ogg_realloc(os->lacing_vals,os->lacing_storage*sizeof(*os->lacing_vals));
			int[] temp = new int[lacing_storage];
			System.arraycopy(lacing_vals, 0, temp, 0, lacing_vals.length);
			lacing_vals = temp;

			// granule_vals =
			// _ogg_realloc(os->granule_vals,os->lacing_storage*sizeof(*os->granule_vals));
	 ...
	 * does not guarantee that all packets have flushed. Only a return value of
	 * 0 from ogg_stream_flush indicates all packet data is flushed into pages.
	 * 
	 * since ogg_stream_flush will flush the last page in a stream even if it's
	 * undersized, you almost certainly want to use ogg_stream_pageout (and
	 * *not* ogg_stream_flush) unless you specifically need to flush an page
	 * regardless of size in the middle of a stream.
	 */
	...
	public boolean ogg_stream_flush(ogg_page og) {
		...
		/*
		 * because someone called stream_reset; this would be a strange thing to
		 * do in an encode stream, but it has plausible uses
		 */
		...
		System.arraycopy(body_data, body_returned, og.body, 0, body_data.length
				- body_returned);
		og.body_len = bytes;
